...
#unigram for positive with add-k smoothing
count_pos = {} 
prob_pos = []
length_pos = 0
...
for key in posWords:
	prob_pos.append((count_pos[key]+k) / (length_pos+(k*len(posWords))))
...
for key in negWords:
	prob_neg.append((count_neg[key]+k) / (length_neg+(k*len(negWords))))

#PERPLEXITY
negative_perplexity = 0.0
for p in prob_neg:
	negative_perplexity -= math.log(p)
negative_perplexity = negative_perplexity / length_neg #summation / N
